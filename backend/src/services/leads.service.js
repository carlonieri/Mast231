...
// categoria/data dell'ultimo contatto. Lo stato passa a 'contattato' se era
// 'da_contattare' (primo contatto) o 'senza_risposta' (ricontattato: un nuovo
// invio è un nuovo tentativo, deve rientrare nel ciclo di flag-no-response-
// leads.job.js per i prossimi N giorni, non restare bloccato in
// 'senza_risposta' per sempre) — non sovrascrive invece stati assegnati da
// una risposta umana (interessato/non_interessato/escluso).
  ...
  const result = await getPool().query(
    `INSERT INTO leads (email, stato, ultima_categoria_email, ultima_data_contatto)
     ...
     ON CONFLICT (email) DO UPDATE SET
       stato = CASE WHEN leads.stato IN ('da_contattare', 'senza_risposta') THEN 'contattato' ELSE leads.stato END,
